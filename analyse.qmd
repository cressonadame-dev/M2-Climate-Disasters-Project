...
df[, date  := as.IDate(sprintf("%04d-%02d-01", year, month))]

# Transformations (stabiliser les variables très asymétriques)
df[, ln_dommages := log1p(dommages_dollars)]
df[, ln_morts    := log1p(morts_moyen)]

# Filtre minimal
df <- df[!is.na(temperature_moyenne) & !is.na(nb_desastres)]
```
